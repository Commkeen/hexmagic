...
public class PolyCellMeshGenerator
{
    ...
    private void MeshifyCellSegment(PolyCell cell, int side)
    {
        ...
        Debug.Assert(prevNeighbor != null);
        Debug.Assert(nextNeighbor != null);
                               ...
                               bool left, Color color)
    {
        ...
        var innerCornerElevated = innerCorner + Vector3.up*innerElevationFactor;
        var outerCornerElevated = outerCorner + Vector3.up*fwdElevationFactor;
        var bridgePointElevated = bridgePoint + Vector3.up*fwdElevationFactor;
        ...
        {
            if (left) {endLeft = outerCorner+ Vector3.up*otherElevationFactor;}
            if (!left) {endRight = outerCorner+ Vector3.up*otherElevationFactor;}
            AddTriangle(begin*radius, endLeft*radius, endRight*radius, color);
            return;
        }
        ...
        {
            begin = outerCorner + Vector3.up*otherElevationFactor;
            endLeft = left ? bridgePointElevated : innerCornerElevated;
            endRight = left ? innerCornerElevated : bridgePointElevated;
            var leftUp = left ? bridgePointElevated.normalized : cell.GetUpVector();
            var rightUp = left ? cell.GetUpVector() : bridgePointElevated.normalized;
        ...
        if (otherIsCliff && fwdIsTerrace)
        {
            ...
            var outerMidpoint = Vector3.Lerp(innerCornerElevated, outerCorner + Vector3.up*otherElevationFactor, 0.5f);
            ...
            AddTriangle(endLeft*radius, outerCorner + Vector3.up*otherElevationFactor*radius, endRight*radius, color);
            return;
        }
        AddTriangle(begin*radius, endLeft*radius, endRight*radius, color);
    }
    ...
    private void MeshifyCornerTerrace(Vector3 begin, PolyCell beginCell,
                                       ...
                                       Color color)
    {
        MeshifyCornerInternal(begin, beginCell, endLeft, beginCell.GetUpVector(), true,
                                                endRight, beginCell.GetUpVector(), true, color);
    }
    ...
    private void MeshifyInnerCornerTerrace(PolyCell cell, Vector3 begin, Vector3 ownTop, Vector3 sharedTop, bool left, Color color)
    {
        if (left)
        {
            MeshifyCornerInternal(begin, cell, sharedTop, sharedTop.normalized, true, 
                                                ownTop, cell.GetUpVector(), true, color);
        }
        else
        {
            MeshifyCornerInternal(begin, cell, ownTop, cell.GetUpVector(), true, 
